...
spark = SparkSession.builder \
    .appName("DaroogheFraudDetection") \
...
# -------------------------------
# 4. Load customer average amounts (simulate pre-existing data)
...
# 5. Fraud Rule 1: Velocity Check (more than 2 txns in 2 min)
velocity_alerts = df_parsed \
...
# -------------------------------
# 6. Fraud Rule 2: Geographical Impossibility

# Join the dataframe with itself on customer_id within 5 minutes
...
geo_alerts = geo_join \
    .withColumn("distance", haversine_udf(
        col("a.location.lat"), col("a.location.lng"),
        col("b.location.lat"), col("b.location.lng")
    )) \
...
# -------------------------------
# 7. Fraud Rule 3: Amount Anomaly
amount_anomaly_alerts = df_parsed \
    .join(customer_avg_amounts, "customer_id") \
...
# -------------------------------
# 9. Write to Kafka fraud_alerts topic
...
query = output_df.writeStream \
    ...
    .option("checkpointLocation", "/tmp/spark_checkpoint_fraud2") \
    .outputMode("append") \
    .start()
